...
namespace sdcrew.Views.Postflight
{
    ...
    public partial class Checklist : PopupPage, INotifyPropertyChanged
    {
        #region refs

        PostflightServices preflightServices;
        UserServices userServices;
        PostflightRepository preflightRepository;
        ...
        public Checklist(int TripId)
        {
            #region refs

            preflightServices = new PostflightServices();
            userServices = new UserServices();
            preflightRepository = new PostflightRepository();

            ScheduledAircraftTripID = TripId;
        ...
        protected async override void OnAppearing()
        {
            ...
            var t = await preflightServices.AddPreflight_Checklists(ScheduledAircraftTripID, 2);
            if (t == true)
            {
                FIllChecklists();
            }
        ...
        public ObservableCollection<FlightChecklist> GetScheduleList()
        {
            ...
            var filteredTails = preflightServices.GetAll_ChecklistsByRoleName(ScheduledAircraftTripID, 2, "Scheduling");
        ...
        {
            var filteredTails = preflightServices.GetAll_ChecklistsByRoleName(ScheduledAircraftTripID, 2, "Maintenance");
            bool getHeaderVisibility = false;
            var getSignedID = filteredTails.Select(x => x.checklistSignId).FirstOrDefault();
            if (getSignedID != 0)
            {
        ...
        public ObservableCollection<FlightChecklist> GetPilotchecklist()
        {
            ...
            var filteredTails = preflightServices.GetAll_ChecklistsByRoleName(ScheduledAircraftTripID, 2, "Pilot");
            bool getHeaderVisibility = false;
            var getSignedID = filteredTails.Select(x => x.checklistSignId).FirstOrDefault();
            if (getSignedID != 0)
            {
                ...
                else if (action == "e-Sign")
                {
                    ...
                    if (getEsignPin != null && getEsignPin != "")
                    {
                        ...
                        if (ActiveTab >= 2)
                        {
                            ...
                            getSavedList = preflightServices.Get_SelectedChecklists(selectedMaintenanceIds, "Maintenance", ScheduledAircraftTripID);
                        }

                        else if (ActiveTab == 1)
                        {
                            checklistRoleTypeId = 1;
                            getSavedList = preflightServices.Get_SelectedChecklists(selectedScheduleIds, "Scheduling", ScheduledAircraftTripID);
                        ...
                        else if (ActiveTab == -1 || ActiveTab == 0)
                        {
                            ...
                            getSavedList = preflightServices.Get_SelectedChecklists(selectedPilotIds, "Pilot", ScheduledAircraftTripID);
                        }

                        var getESignStatus = await preflightRepository.postChecklistESign(int.Parse(getEsignPin), FlightId, 2, checkListTypeId, checklistRoleTypeId, getSavedList, user.Email, LegNumber);
        ...
        public async Task SaveChecklist()
        {
            ...
            if (ActiveTab == 2)
            {
                ...
                var listMaintenance = preflightServices.Get_SelectedChecklists(selectedMaintenanceIds, "Maintenance", ScheduledAircraftTripID);
            ...
            if (ActiveTab == 1)
            {
                ...
                var listScheduling = preflightServices.Get_SelectedChecklists(selectedScheduleIds, "Scheduling", ScheduledAircraftTripID);
                ...
                foreach (var selected in listScheduling)
                {
                    ...
                    var PostScheduling = preflightRepository.Save_Checklist(VChecklist);
            ...
            if (ActiveTab == -1 || ActiveTab == 0)
            {
                ...
                var listPilot = preflightServices.Get_SelectedChecklists(selectedPilotIds, "Pilot", ScheduledAircraftTripID);
                ...
                foreach (var selected in listPilot)
                {
                    ...
                    var PostList = preflightRepository.Save_Checklist(VChecklist);
                    preflightServices.UpdateChecklistVm(VChecklist);
                }
        ...
        public async Task SaveUnsaveChecklist()
        {
            ...
            if (ActiveTab == 2)
            {
                ...
                var UnselectedlistMaintenance = preflightServices.Get_UnSelectedChecklists(NotselectedMaintenanceIds, "Maintenance", ScheduledAircraftTripID);
                ...
                foreach (var unselected in UnselectedlistMaintenance)
                {
                    ...
                    var PostMaintenance = preflightRepository.Delete_Checklist(VChecklist);
                    preflightServices.UpdateChecklistVm(VChecklist);
                }
            ...
            if (ActiveTab == 1)
            {
                ...
                var UnselectedlistScheduling = preflightServices.Get_UnSelectedChecklists(NotselectedScheduleIds, "Scheduling", ScheduledAircraftTripID);
            ...
            if (ActiveTab == -1 || ActiveTab == 0)
            {
                ...
                var UnselectedlistPilot = preflightServices.Get_UnSelectedChecklists(NotselectedPilotIds, "Pilot", ScheduledAircraftTripID);
