...
namespace sdcrew.Views.Preflight
{
    ...
    public partial class PreflightDetails : ContentPage, INotifyPropertyChanged
    {
        ...
        public PreflightDetails(object flightObj)
        {
            ...
            //SubNavCollection.ItemsSource = AllTabs;
        ...
        {
            string getIteneraryName = flight.tailNumber + "-" + flight.scheduledAircraftTripId + "-" + flight.legNumber;
            ...
            else if (getType == "View Pax Itinerary") { type = 2; pfDetailControls.IsEnabled = false; }
            else
            {
                MainThread.BeginInvokeOnMainThread(() => Loader.IsVisible = false);
                return;
            ...
            await Task.Run(async () => await preflightRepository.Fetch_Itineray(type, legNumber, CrewIDarrayList, BusinessPersonIdarrayList, flight.scheduledAircraftTripId, getIteneraryName))
                .ContinueWith(x =>
                {
