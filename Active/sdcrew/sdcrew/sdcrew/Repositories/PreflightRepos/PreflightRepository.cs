...
namespace sdcrew.Repositories.PreflightRepos
{
    public class PreflightRepository
    {
        ...
        public async Task<IEnumerable<PreFlight>> GetPreflightAircraftEvents()
        {
            ...
            string getStartDate = DateTime.Today.AddDays(-7).ToShortDateString();
            string getEndDate = DateTime.Today.AddDays(+30).ToShortDateString();
        ...
        public async Task Fetch_Itineray(int type, int legNumber, ArrayList crewMemberIds, ArrayList passengerBusinessPersonIds, int scheduledAircraftTripId, string getIteneraryName)
        {
            ...
            try
            {
                await requestsService.PostAsyncPDF(url, requestBody, getIteneraryName);
